...
class EstadoCivilController extends Controller
{
    public function view() {
        return view('modules.EstadoCivil.view');
    }
     
    public function index() {
        $item = EstadoCivil::select('id', 'Num', 'EstadoCivil')->whereNull('deleted_at');
        
        if(\Auth::user()->Rol != 1) {
            $item = $item->where('id', \Auth::user()->EstadoCivil);
        }

        return Datatables::of($item)
            ->addColumn('action', function ($p) {
                return '<a href="#" @click.prevent="showEstadoCivil('. $p->id . ')" class="btn btn-info btn-xs"><i class="fa fa-bars"></i> '. trans('labels.actions.details') .'</a> &nbsp;';
            })
        ->editColumn('id', '{{$id}}')
        ->make(true);
    }
